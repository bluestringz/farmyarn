...
const UI = (() => {
  ...
  function renderShop(catalog, activeCategory, player, onBuy, onCategoryChange, onDye) {
    ...
    if (activeCategory === 'outfits') {
      ...
      const equippedOutfit = items.find((o) => o.equipped);
      const dyeSwatches = (catalog.dyePalette || []).map((color) =>
        `<button class="dye-swatch" data-color="${color}" style="background:${color}"></button>`
      ).join('');
      const dyeSection = equippedOutfit ? `
        <div class="panel-section-title">Dye your shirt (🪙 ${catalog.dyeCost || 25})</div>
